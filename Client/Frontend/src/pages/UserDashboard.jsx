  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-[#181c2f] via-[#232946] to-[#0f111a] font-sans text-white">
      ...
      <aside className="w-64 bg-[#181c2f]/80 backdrop-blur-lg border-r border-white/10 flex flex-col p-6 space-y-8 shadow-2xl">
        <div className="text-2xl font-extrabold text-[#eebbc3] mb-8 tracking-tight cursor-pointer" onClick={() => navigate('/')}>
          NeuroBridge
        </div>
        <nav className="flex flex-col gap-4">
          <button onClick={() => navigate('/dashboard')} className="text-white hover:text-[#eebbc3] font-medium text-left transition">Dashboard</button>
          <button onClick={() => navigate('/profile')} className="text-white hover:text-[#eebbc3] font-medium text-left transition">Profile</button>
          <button onClick={() => navigate('/settings')} className="text-white hover:text-[#eebbc3] font-medium text-left transition">Settings</button>
        </nav>
        <div className="mt-auto flex items-center gap-3">
          <img src={user.avatar || "https://ui-avatars.com/api/?name=" + encodeURIComponent(user.name)} alt="avatar" className="w-10 h-10 rounded-full border-2 border-[#eebbc3]/60" />
          <div>
